...
const canCancel = computed(() => booking.value && !cancelled.value && !['completed', 'checked_in', 'checked_out'].includes(String(booking.value.status).toLowerCase()) && new Date(booking.value.checkin ?? booking.value.check_in) > new Date())
...
function statusText(status) { return ({ pending: 'Chờ xác nhận', confirmed: 'Đã xác nhận', cancelled: 'Đã hủy', canceled: 'Đã hủy', checked_in: 'Đã nhận phòng', checked_out: 'Đã trả phòng', completed: 'Đã hoàn tất', paid: 'Đã thanh toán', succeeded: 'Thành công', failed: 'Thất bại', refunded: 'Đã hoàn tiền' })[String(status).toLowerCase()] ?? status ?? 'Đã tiếp nhận' }
...
async function fetchBooking() {
  ...
  try {
    booking.value = responseData(await api.get(`/bookings/${route.params.code}`, { params: { email: email.value } }))
    const [paymentResult, invoiceResult] = await Promise.allSettled([
      api.get(`/booking/${route.params.code}/payments`, { params: { email: email.value } }),
      api.get(`/booking/${route.params.code}/invoice`, { params: { email: email.value } }),
    ])
    ...
    sessionStorage.setItem('staygo_booking_email', email.value); localStorage.setItem('staygo_booking_email', email.value)
  ...
  <div class="booking-detail-page"><div class="container booking-detail-container">
    ...
    <template v-else-if="booking">
      ...
      <div class="detail-layout"><main>
        ...
        <section class="detail-card"><h2>Chi tiết lưu trú</h2><div class="stay-grid"><div><small>Nhận phòng</small><strong>{{ booking.checkin ?? booking.check_in }}</strong><span>Từ 14:00</span></div><div><small>Trả phòng</small><strong>{{ booking.checkout ?? booking.check_out }}</strong><span>Trước 12:00</span></div><div><small>Khách</small><strong>{{ booking.guest_name }}</strong><span>{{ booking.adults ?? 2 }} người lớn · {{ booking.children ?? 0 }} trẻ em</span></div><div><small>Số phòng</small><strong>{{ booking.rooms_count ?? booking.rooms_quantity ?? 1 }}</strong><span>{{ booking.nights }} đêm</span></div></div></section>
